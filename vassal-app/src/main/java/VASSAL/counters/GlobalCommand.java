    ...
    try {
      ...
      if (target.fastMatchLocation) {
        ...
        case CURLOC:
          ...
          break;
        case ZONE:
          fastZone = target.targetZone.getText(source, owner, "Editor.GlobalKeyCommand.zone_name");
          break;
        case DECK:
          fastDeck = target.targetDeck.getText(source, owner, "Editor.GlobalKeyCommand.deck_name");
          break;
        case LOCATION:
          fastLocation = target.targetLocation.getText(source, owner, "Editor.GlobalKeyCommand.location_name");
          break;
        case XY:
          fastBoard = target.targetLocation.getText(source, owner, "Editor.GlobalKeyCommand.board_name");
          fastX = target.targetX.getText(source, owner, "Editor.GlobalKeyCommand.x_position");
          fastY = target.targetY.getText(source, owner, "Editor.GlobalKeyCommand.y_position");
          break;
        }

        if (!target.targetType.isCurrent()) {
          fastMap = target.targetMap.getText(source, owner, "Editor.GlobalKeyCommand.map_name");
        }
      }
      ...
      if (target.fastMatchProperty) {
        fastProperty = target.targetProperty.getText(source, owner, "Editor.GlobalKeyCommand.property_name");
        fastValue    = target.targetValue.getText(source, owner, "Editor.GlobalKeyCommand.property_compare");
        if ((target.targetCompare == GlobalCommandTarget.CompareMode.EQUALS) ||
          ...
          try {
            ...
            ErrorDialog.dataWarning(new BadDataReport("Fast Match - syntax error in regex: ", target.targetValue.getFormat()));  //NON-NLS
