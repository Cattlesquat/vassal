...
[cols=",",]
|===
|image:images/CounterDetailViewer.png + |*Description:* An optional description which will be displayed only in the Editor. For use distinguishing between multiple Stack Viewers.

*Recommended Delay before display:*  When the mouse has been stationary for this many milliseconds, the viewer will appear.
...
*Center viewer over mouse position?* If checked, the rollover text which appears will be horizontally centered over top of the current mouse position. Otherwise it will prefer to appear to the right.
...
*Additional offset upward (entire viewer):*  Can be used to adjust the location that the Stack Viewer appears upward (positive values) or downward (negative values).

*Display text:*  If selected, then the viewer will draw some summary text and some individualized text for each piece.

*Enable HTML and "Quick Colors" in text?*  If selected, HTML tags can be included in the text to be displayed, e.g. to add tags for bold/italic and/or other stylings. In addition, the <<ChatLog#QuickColors,Quick Colors>> characters (|,!,?,~, and `) can be included as the very first character in order to easily select alternate colors.

*Center text over pieces?*  If selected, the summary text which appears above the group of pieces being summarized will be centered. Otherwise it will prefer to start at the upper left corner.

*Stretch summary text box to width of pieces?*  If selected, the summary text box which appears above the group of pieces will have its width stretched to match that of the group of piece images.

*Extra text box padding:*  Adds extra pixels of padding to the edges of any text boxes.

*Additional Offset Upward (summary text):*  Can be used to adjust finetune the position of the summary text upward (positive values) or downward (negative values).
...
The numeric values of this property for all included pieces will be substituted. If the *HTML and Quick Colors* option above is checked, then HTML
tags and <<ChatLog#QuickColors,Quick Colors>> characters can be included as well.

*Text below each piece:*  A <<MessageFormat.adoc#top,Message Format>> specifying the text to display below each included piece. If the *HTML and Quick Colors* option above is checked, then HTML
tags and <<ChatLog#QuickColors,Quick Colors>> characters can be included as well.
...
*Include move-when-selected pieces?*  If checked, pieces whose <<NonStacking.adoc#top,Does not stack>> trait only allows them to move when selected will be included; otherwise, they will be excluded.

*Include non-movable pieces?*  If selected, non-movable pieces will be included; if not, they will be excluded.
...
*Include top pieces in Deck:* If selected, then the top pieces of a <<Deck.adoc#top,Deck>> are eligible for inclusion.

*Number of pieces in Deck to show* If the top pieces of a Deck are to be included with the above option, this field determines the maximum amount of pieces from the deck will be summarized.
